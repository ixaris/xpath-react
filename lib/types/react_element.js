...
function getChildElementNode(child) {
  var pubInst = child.getPublicInstance && child.getPublicInstance();
  if (pubInst) {
...
XPathReactElement.prototype.getChildNodes = function() {
  ...
  if (ReactInternals.isDOMComponent(this.getNativeNode())) {
    ...
    if (domInternalInstance._renderedChildren) {
      ...
      for (var key in domInternalInstance._renderedChildren) {
        ...
        if (renderedChildren[key]._stringText) {
          ...
          var childNode = getChildElementNode(renderedChildren[key]);	
          if (childNode) {
            children.push(new XPathReactElement(childNode, this, children.length));  
          } 
        }
      }
    ...
    if (internalInstance._renderedComponent) {
      var childNode = getChildElementNode(internalInstance._renderedComponent);	
      if (childNode) {
        children.push(new XPathReactElement(childNode, this, 0));  
      } 
    }
